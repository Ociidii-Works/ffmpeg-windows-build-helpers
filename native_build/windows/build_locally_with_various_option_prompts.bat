...
IF NOT EXIST ffmpeg_local_builds\cygwin_local_install (
	...
	FOR /F "tokens=4,5 delims=[.XP " %%A IN ('VER') DO (
		IF %%A.%%B==5.1 (
			...
			--root !cd! ^
			--packages ^
			ed,curl,libcurl4,wget,subversion,texinfo,gcc-g++,bison,flex,cvs,yasm,automake,libtool,autoconf,autogen,gcc-core,cmake,git,make,pkg-config,zlib1g-dev,mercurial,unzip,pax,ncurses,patch,gettext-devel,nasm,p7zip,gperf,m4,libtool
		) ELSE (
